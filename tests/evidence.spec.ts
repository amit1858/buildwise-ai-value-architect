...
const baseURL = process.env.EVIDENCE_BASE_URL ?? "http://localhost:3001";
...
test.describe("BuildWise final evidence", () => {
  ...
  test("demo journey and evidence capture", async ({ browser }) => {
    ...
    await expect(page.getByText(/No paid provider call was made/i).first()).toBeVisible();
    ...
    await expect(page.getByText(/Provider keys cannot be entered/i)).toBeVisible();
    ...
    for (const viewport of viewports.slice(1)) {
      ...
      await page.goto(`${baseURL}${projectPath}/blueprint`, { waitUntil: "domcontentloaded" });
