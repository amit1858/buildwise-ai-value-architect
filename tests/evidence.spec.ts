...
test.describe("BuildWise final evidence", () => {
  ...
  test("demo journey and evidence capture", async ({ browser }) => {
    ...
    await page.getByRole("button", { name: /run mocked BYOK test/i }).click();
    await expect(page.getByText(/Mock adapter result/i)).toBeVisible();
    await page.screenshot({ path: path.join(outputDir, `mock-result-${viewports[0].name}.png`), fullPage: true });
