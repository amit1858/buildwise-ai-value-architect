  ...
  const artifacts: BuildArtifact[] = [
    {
      ...
      content: sanitizeSecrets([
        ...
        "",
        `## Value hypothesis`,
        `The ${project.name} workflow targets ${project.input.businessOutcome || "the stated business outcome"} across ${project.input.executionsPerMonth.toLocaleString()} business executions per month while staying inside a ${project.input.monthlyBudget ? `$${project.input.monthlyBudget.toLocaleString()}` : "defined budget"} envelope.`,
        "",
        `## Constraints`,
    ...
    },
    {
      ...
      content: sanitizeSecrets([
        ...
        "- Routing based on explicit metadata and versioned business rules.",
    ...
    },
    {
      ...
      content: sanitizeSecrets([
        ...
        ...project.tasks.map((task) => `- ${task.name}: ${task.humanReviewPolicy}`),
    ...
    },
    {
      ...
      content: sanitizeSecrets([
        ...
        "| --- | --- | --- | --- | --- |",
        `| Unsupported ${project.spine.workloadCategory.toLowerCase()} output | High | Medium | Restrict to relevant evidence and require source tracing | Product owner |`,
